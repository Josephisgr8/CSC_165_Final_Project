...
public class GameServerUDP extends GameConnectionServer<UUID>
{
    ...
    public void processPacket(Object o, InetAddress senderIP, int sndPort){
        ...
        if(msgTokens.length > 0) {
            ...
            //case where server recieves rotate message
            if (msgTokens[0].compareTo("rotate") == 0){
                UUID clientID = UUID.fromString(msgTokens[1]);
                String rotateAmount = msgTokens[2];
                sendRotateMessages(clientID, rotateAmount);
            }
            //case where server receives skin message
            if (msgTokens[0].compareTo("skin") == 0){
                UUID clientID = UUID.fromString(msgTokens[1]);
                sendChangeSkinMessages(clientID);
            }
        }
    }
    ...
    public void sendRotateMessages(UUID clientID, String amt){
        // etc.....  Tell everyone that this client just rotated
        try {
    ...
    public void sendChangeSkinMessages(UUID clientID){
        // etc.....  Tell everyone that this client just changed skin
        try {
            String message = new String("skin," + clientID.toString());
            forwardPacketToAll(message, clientID);
        }
        catch (IOException e) {e.printStackTrace();}
    }
