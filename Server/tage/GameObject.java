...
public class GameObject
{
	...
	*/
	...
	//Custom Methods - Joseph DiMarino
	/** Rotate's the GameObject about its own Y-axis by the given amount as a float. */
	public void yaw(float amountRotate) {
		Matrix4f oldRotation = new Matrix4f(this.getWorldRotation());
        Vector4f rotateAxis = new Vector4f(0f,1f,0f,1f).mul(oldRotation);

        Matrix4f avatarRotationMatrix = new Matrix4f().rotation(amountRotate, 
        new Vector3f(rotateAxis.x(), rotateAxis.y(), rotateAxis.z()));

        Matrix4f newRotation = new Matrix4f(oldRotation);
        avatarRotationMatrix.mul(newRotation);
        this.setLocalRotation(avatarRotationMatrix);
	}

	/** Rotate's the GameObject about its own X-axis by the given amount as a float. */
	public void pitch(float amountRotate) {

		Matrix4f oldRotation = new Matrix4f(this.getWorldRotation());
        Vector4f rotateAxis = new Vector4f(-1f,0f,0f,1f).mul(oldRotation);
	...
	/** Moves the GameObject forward according to its WorldForwardVector by the specified amount as a float. */
	public void moveFwd(float moveAmount) {
		Vector3f fwd, loc, newLocation;

		fwd = this.getWorldForwardVector();
        loc = this.getWorldLocation();
        newLocation = loc.add(fwd.mul(moveAmount));
