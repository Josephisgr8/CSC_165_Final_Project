...
public class Camera
{
	...
	/** Rotates the camera about its own V by the specified amount as a float. */
	public void yaw(float rotateAmount){
		Vector3f newU = new Vector3f(u);
		Vector3f newN = new Vector3f(n);

		newU.rotateAxis(rotateAmount, v.x(), v.y(), v.z());
        newN.rotateAxis(rotateAmount, v.x(), v.y(), v.z());
	...
	/** Rotates the camera about its own U by the specified amount as a float. */
	public void pitch(float rotateAmount) {
		Vector3f newV = new Vector3f(v);
		Vector3f newN = new Vector3f(n);

		newV.rotateAxis(rotateAmount, u.x(), u.y(), u.z());
		newN.rotateAxis(rotateAmount, u.x(), u.y(), u.z());
